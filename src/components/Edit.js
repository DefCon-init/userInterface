...
function Edit(props) {
  const [user, setUser] = useState({ _id: '', firstname: '', lastname: '', gender: '', dob: new Date() });
  const [showLoading, setShowLoading] = useState(true);
  const fetchUrl = "http://localhost:5000/users/getUser/" + props.match.params.id;
  const editUrl = "http://localhost:5000/users/updateUser";
  ...
  const updateUser = (e) => {
    setShowLoading(true);
    e.preventDefault();
    const data = { firstname: user.firstname, lastname: user.lastname, gender: user.gender, dob: user.dob, id: user._id };
